  ...
  implements IUniverseRenderer, IRendererController
{
  ...
  drawGrid(): void {
    ...
    this.two.makeLine(
      0,
      0,
      this.worldContainer.translation.x + window.innerWidth,
      0
    );
    this.two.makeLine(
      0,
      0,
      0,
      this.worldContainer.translation.y + window.innerHeight
    );
  }
  ...
  scale(factor: number, origin: Vector2D): void {
    const localPos = {
      x:
        (origin.x - this.worldContainer.translation.x) /
        Number(this.worldContainer.scale),
      y:
        (origin.y - this.worldContainer.translation.y) /
        Number(this.worldContainer.scale),
    };

    this.worldContainer.scale = Number(this.worldContainer.scale) * factor;

    this.worldContainer.translation.x =
      origin.x - localPos.x * this.worldContainer.scale;
    this.worldContainer.translation.y =
      origin.y - localPos.y * this.worldContainer.scale;
  }
